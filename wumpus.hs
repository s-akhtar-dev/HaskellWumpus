...
-- Base case: if 'W' is found, return the path.
if (findByIndices cave row col) == 'W' then
path
-- Move right if within bounds, the cell is not a pit, and
it hasnt been visited.
else if row > 0 && (findByIndices cave (row - 1) col /=
'P') && (findByIndices visited (row - 1) col /= 'y') then
solver cave (replaceByIndices visited row col 'y')
(row - 1) col (path ++ "u")
-- Move left if within bounds, the cell is not a pit, and
it hasnt been visited.
else if row < 3 && (findByIndices cave (row + 1) col /=
'P') && (findByIndices visited (row + 1) col /= 'y') then
solver cave (replaceByIndices visited row col 'y')
(row + 1) col (path ++ "d")
-- Dead end: If no valid moves, mark the current cell as a
pit and restart.
else
solver (replaceByIndices cave row col 'P')
[['n','n','n','n','n','n'],
['n','n','n','n','n','n'],
['n','n','n','n','n','n'],
['y','n','n','n','n','n']] 3 0 []
...
['O', 'O', 'O', 'P', 'O', 'P'],
['O', 'O', 'O', 'O', 'O', 'O']]
-- Main function to run the findWumpus function on cave1 and
